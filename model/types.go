...
type ConsumedRestService struct {
	...
	// OpenApiContent holds the raw OpenAPI JSON/YAML text stored in the
	// Rest$OpenApiFile sub-document. Empty string when the service was created
	// by hand (not imported from a spec). Set by IMPORT REST CLIENT FROM OPENAPI.
	OpenApiContent string `json:"openApiContent,omitempty"`
}
...
type RestClientOperation struct {
	...
	// Tags holds the OpenAPI tags for this operation (informational, from Studio Pro import).
...
type RestClientParameter struct {
	...
	TestValue string `json:"testValue,omitempty"` // Studio Pro test value (empty string when imported from OpenAPI)
}
